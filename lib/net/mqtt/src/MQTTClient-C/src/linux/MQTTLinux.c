...
static void *tls_connect(char *hostname, char *port, int to_ms, Network *ctx)
{
    ...
    if (ctx->pkey_path) {
        ...
        if (pkey_fd) {
...
#ifdef MBEDTLS_V_3_4_0
            ret = mbedtls_pk_parse_key(&tls_ctx->pkey, buf, pkey_len + 1, NULL, 0, mbedtls_ctr_drbg_random, &tls_ctx->ctr_drbg);
#else
            ...
            buf[ctx->pkey_len] = '\0';
#ifdef MBEDTLS_V_3_4_0
            ret = mbedtls_pk_parse_key(&tls_ctx->pkey, buf, ctx->pkey_len + 1, NULL, 0, mbedtls_ctr_drbg_random, &tls_ctx->ctr_drbg);
#else
...
static int socks_read(Network *n, unsigned char *buf, int len, unsigned long ts)
{
    ...
    while (bytes < len) {
        int rc = mbedtls_ssl_read(&tls_ctx->ssl, &buf[bytes], (len - bytes));
        if (rc > 0) {
            bytes += rc;
