...
#if JL_LWIP && LWIP_DHCP_DOES_ACD_CHECK
            struct netif *netif = net_get_netif_handle(lwip_netif);
            if (netif) {
                ip4_addr_t static_ipaddr;
                IP4_ADDR(&static_ipaddr, lan_setting_info->WIRELESS_IP_ADDR0, lan_setting_info->WIRELESS_IP_ADDR1, lan_setting_info->WIRELESS_IP_ADDR2, lan_setting_info->WIRELESS_IP_ADDR3);
                void static_ip_acd_start(struct netif * netif, ip4_addr_t ipaddr);
                static_ip_acd_start(netif, static_ipaddr);
            }
#endif
