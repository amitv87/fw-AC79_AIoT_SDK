    ...
    if (enable_tls) {
        ...
        if (options && options->private_key) {
            ...
            if ((ret = mbedtls_pk_parse_key(&conn->ssl_ctx->pkey, (const unsigned char *)options->private_key, \
                                            options->private_key_size, NULL, 0, mbedtls_ctr_drbg_random, &conn->ssl_ctx->ctr_drbg)))
#else
            if ((ret = mbedtls_pk_parse_key(&conn->ssl_ctx->pkey, (const unsigned char *)options->private_key, \
...
#ifdef MBEDTLS_V_3_4_0
        if ((ret = mbedtls_pk_parse_key(&conn->ssl_ctx->pkey, (const unsigned char *) privateKey, privateKey_size, NULL, 0, mbedtls_ctr_drbg_random, &conn->ssl_ctx->ctr_drbg))) {
#else
