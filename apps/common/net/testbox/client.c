...
int wifibox_client_init(void)
{
    ...
    if (__this->wbcp_ops->init(WB_CLIENT_MODE) != 0) {
...
void wifibox_dut_config(void)
{
    if (wifibox_dut_read_config_info() == 0) {
        log_info("DUT SET : PA = %d,%d,%d,%d,%d,%d; XOSC_L = %d, XSOC_R = %d.\n", dut_pa[0], dut_pa[1], dut_pa[2], dut_pa[3], dut_pa[4], dut_pa[5], dut_xosc_cls, dut_xosc_crs);
        //衰减器初始化
