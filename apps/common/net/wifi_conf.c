...
const char WL_RX_DEBUG = 0; //WIFI底层收繁忙打印, 默认打开检测,生产时关闭
...
#define ICMP_ER   0    /* echo reply */
    if (iph->iphd.protocol == 1 && *((u8 *)iph + sizeof(struct iphdr_e)) != ICMP_ER) { //如果是ICMP并且不是echo reply就丢弃, 因为是一些Time_Ex等无用包
        putbyte('I');
        return -1;
    }
    /*------------------------------------------------------------------------------------------------------------------------------------------------------------*/
