...
/* 大量数据传输时可以设置这个间隔或者缩短休眠间隔 */
void wl_set_active_wait_interval(u8 count)
{
    wl_active_wait_interval_count = count;
    return;
}

u8 wl_get_active_wait_interval(void)
{
    return wl_active_wait_interval_count;
}

/* 设置自定义 beacon 监听间隔，这个间隔时间只有 STA 自己知道，太长可能导致断连 */
void wl_set_custom_listen_interval(u16 interval)
{
    wl_custom_listen_interval = interval;
    return;
}

u16 wl_get_custom_listen_interval(void *priv)
{
    return wl_custom_listen_interval;
}

/* 设置默认 beacon 监听间隔，这个间隔时间在连接关联时会同步给 AP，因此需要在建立连接之前设置，当不监听 DTIM 时，系统按照这个间隔休眠 */
void wl_set_default_listen_interval(u16 interval)
{
    wl_default_listen_interval = interval;
    return;
}

u16 wl_get_default_listen_interval(void)
{
    return wl_default_listen_interval;
}
...
*/
extern bool wl_listen_dtim_enabled_flag;					// 默认情况下监听 DTIM
extern bool wl_listen_each_tim_enabled_flag;
extern bool wl_listen_custom_intervals_enabled_flag;
extern bool wl_listen_intervals_cheak_flag;				// 监听策略修改之后，要给这个标志位置1，让wifi协议栈去更新

void wl_enabled_listen_dtim(void)
{
    wl_listen_dtim_enabled_flag = true;
    wl_listen_each_tim_enabled_flag = false;
    wl_listen_custom_intervals_enabled_flag = false;
    wl_listen_intervals_cheak_flag = true;
    return;
}

// 按照默认监听间隔进行监听，该默认间隔需要在连接之前通过 wl_set_default_listen_interval 配置下去，否则默认值是3
void wl_disenabled_listen_dtim(void)
{
    wl_listen_dtim_enabled_flag = false;
    wl_listen_each_tim_enabled_flag = false;
    wl_listen_custom_intervals_enabled_flag = false;
    wl_listen_intervals_cheak_flag = true;
    return;
...
// 按照自定义的间隔进行监听，该自定义间隔需要通过 wl_set_custom_listen_interval 配置下去，否则默认值是10
void wl_enabled_listen_custom_intervals(void)
{
    wl_listen_dtim_enabled_flag = false;
    wl_listen_each_tim_enabled_flag = false;
    wl_listen_custom_intervals_enabled_flag = true;
    wl_listen_intervals_cheak_flag = true;
    return;
}

// 退出自定义间隔的配置之后，默认回到监听 DTIM 的策略，如果要进入默认间隔的配置则需要再调一次 wl_disenabled_listen_dtim
void wl_disenabled_listen_custom_intervals(void)
{
    wl_enabled_listen_dtim();
    return;
}

void wl_enabled_listen_each_tim(void)
{
    wl_listen_dtim_enabled_flag = false;
    wl_listen_each_tim_enabled_flag = true;
    wl_listen_custom_intervals_enabled_flag = false;
    wl_listen_intervals_cheak_flag = true;
    return;
}

// 退出监听每一个间隔的配置之后，默认回到监听 DTIM 的策略，如果要进入默认间隔的配置则需要再调一次 wl_disenabled_listen_dtim
...
///==================================================
    ...
    /*240*/{ 0x00000078, 0x17830871},
    /*241*/{ 0x00000078, 0x17831871},
    /*242*/{ 0x00000078, 0x17832871},
    /*243*/{ 0x00000078, 0x17834871},
    /*244*/{ 0x00000078, 0x17836871},
    /*245*/{ 0x00000078, 0x17A36871},
    /*246*/{ 0x00000078, 0x17E36871},
    /*247*/{ 0x00000078, 0x57E36871},
    /*248*/{ 0x00000078, 0x97E36871},
    /*249*/{ 0x00000078, 0xD7E36875},
    /*250*/{ 0x00000078, 0xD7E36876},
    /*251*/{ 0x00000078, 0xD7E3687a},
    /*252*/{ 0x00000078, 0xD7E3687a},
    /*253*/{ 0x00000078, 0xD7E3687a},
    /*254*/{ 0x00000078, 0xD7E3687a},
    /*255*/{ 0x00000078, 0xD7E3687a},
};
