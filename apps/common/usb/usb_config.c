...
#if TCFG_HOST_UVC_ENABLE || TCFG_USB_SLAVE_UVC_ENABLE
#define     VIDEO_DMA_SIZE  (UVC_FIFO_TXMAXP * UVC_PKT_SPILT+2)
#else
...
struct usb_ep_dmabuf_t {
    u8 ep0_dma_buffer[EP0_SETUP_LEN];
    u8 hid_dma_rx_buffer[HID_DMA_SIZE];
    u8 hid_dma_tx_buffer[HID_DMA_SIZE];
    u8 spk_dma_buffer[AUDIO_DMA_SIZE];
    u8 mic_dma_buffer[AUDIO_DMA_SIZE];
    u8 msd_dma_buffer[2][MSD_DMA_SIZE];
