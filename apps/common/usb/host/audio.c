...
{
    struct usb_host_device *host_dev = (struct usb_host_device *)host_id2device(__this->usb_id);
    u8 featureUnitID = 6;
...
static void usb_audio_event_handler(struct sys_event *sys)
{
    struct device_event *event = (struct device_event *)sys->payload;
    const char *audio = (const char *)event->value;
