        ...
        if (cur_type == USB_DT_INTERFACE_ASSOCIATION) {
            ...
            if ((end_desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK) {
                if (end_desc->bEndpointAddress & USB_DIR_IN) {
                    ...
                    log_info("AT IN : end_desc->wMaxPacketSize = %d, end_desc->bInterval = %d", end_desc->wMaxPacketSize, end_desc->bInterval);
                    if (usb_id == 0) {
                    ...
                    log_info("AT OUT : end_desc->wMaxPacketSize = %d", end_desc->wMaxPacketSize);
                    if (usb_id == 0) {
...
#endif
                            rx_interval = end_desc->bInterval;
                            log_info("cdc IN : end_desc->wMaxPacketSize = %d, end_desc->bInterval = %d", end_desc->wMaxPacketSize, end_desc->bInterval);
                        } else {
...
#endif
                            log_info("cdc OUT : end_desc->wMaxPacketSize = %d", end_desc->wMaxPacketSize);
                        }
                    }
                }
