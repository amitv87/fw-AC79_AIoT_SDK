...
s32 usbnet_generic_cdc_parser(struct usb_host_device *host_dev, u8 interface_num, const u8 *pBuf)
{
    ...
    udisk_inf.dev.wireless->parent = host_dev;
    ...
    while (loop) {
        ...
        case USB_CDC_UNION_TYPE:
            ...
            if (buf[1] == USB_DT_ENDPOINT) {
                ...
                usb_h_set_ep_isr(host_dev, wireless_dev.host_epin | USB_DIR_IN, usbnet_rx_isr, &udisk_inf);
                u8 *ep_buffer = usb_h_get_ep_buffer(usb_id, wireless_dev.host_epin | USB_DIR_IN);
                usb_h_ep_config(usb_id, wireless_dev.host_epin | USB_DIR_IN, USB_ENDPOINT_XFERTYPE_MASK, 1, rx_interval, ep_buffer, 64);

                ep_buffer = usb_h_get_ep_buffer(usb_id, wireless_dev.host_epout | USB_DIR_OUT);
                usb_h_ep_config(usb_id, wireless_dev.host_epout | USB_DIR_OUT, USB_ENDPOINT_XFERTYPE_MASK, 0, 0, ep_buffer, 64);

                usb_h_ep_read_async(usb_id, wireless_dev.host_epin, wireless_dev.epin, NULL, 0, USB_ENDPOINT_XFER_INT, 1);
            }
            return len;
            break;
