...
static struct mass_storage mass_stor[2];//SEC(.usb_h_udisk);

static const struct usb_interface_info udisk_inf[2] = {
    {
...
static struct device udisk_device[2];//SEC(.usb_h_udisk);
...
int usb_msd_parser(struct usb_host_device *host_dev, u8 interface_num, const u8 *pBuf)
{
    ...
    u8 *ep_buffer = usb_h_get_ep_buffer(usb_id, disk->udisk_ep.host_epin | USB_DIR_IN);
    usb_h_ep_config(usb_id, disk->udisk_ep.host_epin | USB_DIR_IN, USB_ENDPOINT_XFER_BULK, 0, 0, ep_buffer, 64);


    ep_buffer = usb_h_get_ep_buffer(usb_id, disk->udisk_ep.host_epout | USB_DIR_OUT);
    usb_h_ep_config(usb_id, disk->udisk_ep.host_epout | USB_DIR_OUT, USB_ENDPOINT_XFER_BULK, 0, 0, ep_buffer, 64);
    return len;
}
/**
 * @brief usb_stor_open
