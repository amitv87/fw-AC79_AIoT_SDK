...
static int _usb_uvc_parse(struct usb_host_device *host_dev, u8 interface_num, const u8 *pBuf)
{
    ...
    return usb_uvc_parser(host_dev, (u8 *)pBuf, interface_num);
#else
    return USB_DT_INTERFACE_SIZE;
#endif
}
...
int usb_host_class_query(const usb_dev usb_id, u32 dev_class)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (host_dev->interface_info[i] && host_dev->interface_info[i]->ctrl) {
            if (host_dev->interface_info[i]->ctrl->interface_class == dev_class) {
                ret = 1;
                break;
            }
        }
