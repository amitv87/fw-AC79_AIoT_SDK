...
void usb_start()
{
...
#if TCFG_USB_SLAVE_HID_ENABLE
#ifdef CONFIG_ANC_PC_ENABLE
    extern void hid_set_usb_dev(usb_dev usb_id);
    extern int hid_recv_handler(u8 * buf, u32 len);
    extern u8 *hid_get_report_desc();
    extern u32 hid_get_report_desc_size();
    hid_set_report_desc(0, hid_get_report_desc(), hid_get_report_desc_size());
    hid_set_output_handle(0, hid_recv_handler);
    hid_set_usb_dev(0);
    ...
    hid_set_report_desc(0, hid_keyboard_get_report_desc(), hid_keyboard_get_report_desc_size());
    hid_set_output_handle(0, hid_keyboard_output);
    hid_keyboard_set_usb_dev(0);
    ...
    hid_set_report_desc(0, hid_pos_get_report_desc(), hid_pos_get_report_desc_size());
    hid_set_output_handle(0, hid_pos_receive);
    hid_pos_open(0);
#endif
#endif
    hid_control(0, 1);
...
#if TCFG_USB_SLAVE_HID_ENABLE
    hid_control(0, 0);
    hid_set_report_desc(0, NULL, 0);
    hid_set_output_handle(0, NULL);
#ifdef CONFIG_ANC_PC_ENABLE
    hid_set_usb_dev(-1);
#endif
