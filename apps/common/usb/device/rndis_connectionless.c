...
#else
/* fs = 64; hs = 512 */
static u8 _cdc_buffer[USB_MAX_HW_NUM][512] SEC(.cdc_rndis_var) __attribute__((aligned(4)));
...
static u32 rndis_setup(struct usb_device_t *usb_device, struct usb_ctrlrequest *ctrl_req)
{
    ...
    case USB_TYPE_CLASS:
        ...
        case USB_CDC_GET_ENCAPSULATED_RESPONSE:
            log_debug("encapsulated response.");
...
static u32 rndis_setup_rx(struct usb_device_t *usb_device, struct usb_ctrlrequest *ctrl_req)
{
    ...
    if (usb_device->wDataLength == 0) {
        ...
        case USB_TYPE_CLASS:
            switch (ctrl_req->bRequest) {
            case USB_CDC_SEND_ENCAPSULATED_COMMAND:
                log_debug("USB_CDC_SEND_ENCAPSULATED_COMMAND");
...
u32 rndis_desc_config(const usb_dev usb_id, u8 *ptr, u32 *itf_num)
{
    ...
    if (usb_id != 0) {
        tptr[8 + 9 + 5 + 5 + 4 + 5 + 6] = 4;  //high-speed mode, 125x2^(4-1)=1ms, bInterval
    }
#endif
...
void rndis_register(const usb_dev usb_id)
{
    ...
    if (!cdc_rndis_hdl[usb_id]) {
        ...
        cdc_rndis_hdl[usb_id]->max_size_cdc_bulkout = MAXP_SIZE_CDC_BULKOUT;
        cdc_rndis_hdl[usb_id]->max_size_cdc_bulkin = MAXP_SIZE_CDC_BULKIN;

        if (usb_id == 0) {
            cdc_rndis_hdl[usb_id]->speed = 12000000;
            cdc_rndis_hdl[usb_id]->max_size_cdc_bulkout = 64;
            cdc_rndis_hdl[usb_id]->max_size_cdc_bulkin = 64;
        } else {
            ...
            cdc_rndis_hdl[usb_id]->max_size_cdc_bulkout = 512;
            cdc_rndis_hdl[usb_id]->max_size_cdc_bulkin = 512;
#else
