...
#ifdef TCFG_EXTFLASH_UDISK_ENABLE
    cnt++;
#endif
...
static void msd_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    ...
    usb_g_ep_config(usb_id, MSD_BULK_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_BULK, 0, msd_handle[usb_id]->ep_out_dmabuffer, usb_id ? MAXP_SIZE_BULKIN_2_0 : MAXP_SIZE_BULKIN);
    usb_g_ep_config(usb_id, MSD_BULK_EP_OUT, USB_ENDPOINT_XFER_BULK, 1, msd_handle[usb_id]->ep_out_dmabuffer, usb_id ? MAXP_SIZE_BULKOUT_2_0 : MAXP_SIZE_BULKOUT);
#else
...
#if defined(CONFIG_CPU_WL82)
    if (usb_id != 0) {
        ptr[13] = LOBYTE(MAXP_SIZE_BULKIN_2_0);
        ptr[14] = HIBYTE(MAXP_SIZE_BULKIN_2_0);
        ptr[20] = LOBYTE(MAXP_SIZE_BULKOUT_2_0);
        ptr[21] = HIBYTE(MAXP_SIZE_BULKOUT_2_0);
    }
#endif
    ptr[2] = *cur_itf_num;
#if TCFG_USB_APPLE_DOCK_EN
...
static void read_capacity(const struct usb_device_t *usb_device)
{
    ...
    dev_ioctl(dev_fd, IOCTL_GET_BLOCK_OFFSET, (u32)&block_offset);
#if !defined CONFIG_DMSDX_ENABLE && defined CONFIG_SDFILE_EXT_ENABLE
    dev_ioctl(dev_fd, IOCTL_GET_BLOCK_SIZE, (u32)&capacity_temp);
    capacity_temp = (CONFIG_SDNAND_HFS_LEN + CONFIG_SDNAND_HFAT_LEN + CONFIG_SDNAND_FAT1_LEN) / capacity_temp;
#endif
...
#if defined(CONFIG_CPU_WL82)
        msd_handle[usb_id]->ep_out_dmabuffer = usb_alloc_ep_dmabuffer(usb_id, MSD_BULK_EP_OUT, usb_id ? MAXP_SIZE_BULKIN_2_0 + MAXP_SIZE_BULKOUT_2_0 : MAXP_SIZE_BULKIN + MAXP_SIZE_BULKOUT);
#else
