...
static void uvc_reset_handle(struct usb_device_t *usb_device, u32 itf)
{
    ...
    u8 *ep_buffer = usb_alloc_ep_dmabuffer(usb_id, UVC_STREAM_EP_IN | USB_DIR_IN, MAXP_SIZE_SLAVE_UVC_BULKIN);
#if UVC_ISO_MODE

    usb_g_ep_config(usb_id, UVC_STREAM_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_ISOC, 1, ep_buffer, (UVC_FIFO_TXMAXP | ((UVC_PKT_SPILT - 1) << 11)));
...
static void usb_video_feature_init(usb_dev usb_id)
{
    ...
    video_ctl_value[usb_id].wHue[UVC_MAX] = 0xff;
    video_ctl_value[usb_id].wHue[UVC_MIN] = 0x00;
    video_ctl_value[usb_id].wHue[UVC_DEF] = 0x80;
    video_ctl_value[usb_id].wHue[UVC_CUR] = 0x80;
