...
SYS_EVENT_STATIC_HANDLER_REGISTER(extflash_device_event) = {
...
int extflash_mount_to_fs_test(void)
{
    ...
    int err = f_format("extflash", FILE_SYSTEM_NAME, 32 * 1024, 0);
    printf("format err : %d", err);

    if (err == 0 && mount("extflash", "mnt/extflash", FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, 0)) {
