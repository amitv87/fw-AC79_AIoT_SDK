    ...
    while (1) {
        if (lg_cnt > last_cnt && last_cnt != lg_cnt) {
            if (lg_cnt / sizeof(lg_buf) > last_cnt / sizeof(lg_buf)) {
                cdc_write_data(CDC_USB_ID, (u8 *)lg_buf + (last_cnt % sizeof(lg_buf)), sizeof(lg_buf) - last_cnt % sizeof(lg_buf));
                cdc_write_data(CDC_USB_ID, (u8 *)lg_buf, lg_cnt - last_cnt - (sizeof(lg_buf) - last_cnt % sizeof(lg_buf)));
            } else {
                cdc_write_data(CDC_USB_ID, (u8 *)lg_buf + (last_cnt % sizeof(lg_buf)), lg_cnt - last_cnt);
            }
        }
        last_cnt = lg_cnt;
        os_time_dly(10);
