...
const char norflash_write_cpu_unhold = 0;//在 跑SFC的情况下 写flash hold cpu,关闭所有中断
#else
const char norflash_write_cpu_unhold = 1;//在跑SDRAM的情况下 写flash 不需要 hold cpu和关闭所有中断 ,程序员自己懂得把相关text段放到sdram/内部ram,保证运行过程中不会访问SFC也可以写1
#endif
