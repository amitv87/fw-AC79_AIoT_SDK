...
void set_lcd_rgb565_data_size(u32 size)
{
    __this.lcd_rgb565_data_size = size;
}
...
void malloc_lcd_buf(void)
{
    if (__this.ui_save_buf == NULL) {
        __this.ui_save_buf = malloc(__this.lcd_rgb565_data_size);
    }
    if (__this.camera_save_buf == NULL) {
        __this.camera_save_buf = malloc(__this.lcd_rgb565_data_size);
    }
    if (__this.show_buf == NULL) {
        __this.show_buf = malloc(__this.lcd_rgb565_data_size);
    }
...
{
    ASSERT(__this.lcd_rgb565_data_size, "__this.lcd_rgb565_data_size = 0");
    u32 buf_size = __this.lcd_rgb565_data_size * lcd_piece;
...
#endif
        send_data_to_lcd(__this.show_buf, __this.lcd_rgb565_data_size);
    }
}
...
static void te_send_task(void)
{
    ...
    while (1) {
        ...
        case UI://ok
            cb_TE_send_lcd_data(__this.out_lcd_buf, __this.lcd_rgb565_data_size);
            break;
        case UI_CAMERA:
            for (u8 i = 0; i < 2; i++) {
                if (task[i].data_ok_flag) {
                    ...
                    cb_TE_send_lcd_data(out_buf_addr, __this.lcd_rgb565_data_size);
...
void ui_send_data_ready(u8 *data_buf, u32 data_size)
{
    ...
    if (__this.data_mode == UI || __this.data_mode == UI_CAMERA) {
        ...
        if (__this.data_mode == UI) {
            send_data_to_lcd(__this.ui_save_buf, __this.lcd_rgb565_data_size);
#if (USE_LCD_TE == 0)
            no_Te_sned_post(0);
#endif
        }
...
void picture_compose_main(u8 id)
{
    ...
    while (1) {
start:
        time_out = os_sem_pend(&task[id].compose_sem, 0);//不能加超时等 防止另一个线程超时发包
        switch (__this.data_mode) {
