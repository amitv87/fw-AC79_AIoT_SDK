...
static int lcd_init(void *p)
{
    ...
    if (!lcd_hdl) {
        ...
        lcd_rgb565_data_size = lcd_w * lcd_h * 2;

        //配置lcd_data_driver一些参数
        set_lcd_data_driver_lcd_h_w(lcd_w, lcd_h);
        set_lcd_rgb565_data_size(lcd_rgb565_data_size);
...
static void lcd_buffer_malloc(u8 **buf, u32 *size)
{
    ...
    *buf = malloc(lcd_dev->lcd_width * lcd_dev->lcd_height * 4);
#else
...
#if UI_USED_DOUBLE_BUFFER
    *size = lcd_dev->lcd_width * lcd_dev->lcd_height * 4;
#else
