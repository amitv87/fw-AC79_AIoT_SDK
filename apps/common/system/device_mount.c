...
int udisk_storage_device_ready(int id)
{
    ...
    if (udisk_online[id]) {
        sta = dev_online(id ? "udsik1" : "udisk0");
        if (!sta) {
            return false;
        }
...
#if TCFG_UDISK_ENABLE
        if (usb_host_class_query(usb_id, USB_CLASS_MASS_STORAGE)) {
            udisk_online[usb_id] = 1;
...
#if TCFG_UDISK_ENABLE
        if (udisk_online[usb_id]) {
            udisk_online[usb_id] = 0;
