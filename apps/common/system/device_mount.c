...
static int mount_sd_to_fs(const char *sd_name)
{
    ...
    const char *sdfile_ext_dev = SDX_DEV;
    if (id == sdfile_ext_dev[2] - '0') {
        ...
        capacity /= 2;	//取当前介质的后一半容量为第二FAT区
        dev_ioctl(fd, IOCTL_GET_BLOCK_SIZE, (u32)&block_size);
#ifndef CONFIG_DMSDX_ENABLE
        dev_ioctl(fd, IOCTL_SET_BLOCK_OFFSET, (u32)&capacity);
#endif
        //根据宏定义配置分区
        u32 fat1_block_offset = (CONFIG_SDNAND_HFS_LEN + CONFIG_SDNAND_HFAT_LEN) / block_size;
#ifndef CONFIG_DMSDX_ENABLE
        dev_ioctl(fd, IOCTL_SET_BLOCK_OFFSET, (u32)&fat1_block_offset);
#endif
        dev_close(fd);
        ...
        //根据宏定义配置分区
        block_offset = ((u64)((CONFIG_SDNAND_HFS_LEN + CONFIG_SDNAND_HFAT_LEN + CONFIG_SDNAND_FAT1_LEN) / block_size) << 32) | ((CONFIG_SDNAND_HFS_LEN + CONFIG_SDNAND_HFAT_LEN) / block_size);
        mt = mount(dev, id ? "storage/sd1" : "storage/sd0", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, block_offset);
        if (!mt) {
            printf("%s mount standard FAT fail, format...\n", sd_name);
            f_format(sd_name, FAT_FILE_SYSTEM_NAME, 32 * 1024, block_offset);
            mt = mount(dev, id ? "storage/sd1" : "storage/sd0", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, block_offset);
        } else {
            printf("%s mount standard FAT suss\n", sd_name);
        }

        block_offset = ((u64)(capacity) << 32) | (__FLASH_SIZE__ / block_size);
        //根据宏定义配置分区
        block_offset = ((u64)((CONFIG_SDNAND_HFS_LEN + CONFIG_SDNAND_HFAT_LEN) / block_size) << 32) | (CONFIG_SDNAND_HFS_LEN / block_size);
        mt = mount(dev, "storage/sdh", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, block_offset);
        if (!mt) {
            printf("%s mount hide FAT fail, format...\n", sd_name);
            f_format(sd_name, FAT_FILE_SYSTEM_NAME, 32 * 1024, block_offset);
            mt = mount(dev, "storage/sdh", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, block_offset);
        } else {
            printf("%s mount hide FAT suss\n", sd_name);
        }
    } else
#endif
    {
        mt = mount(dev, id ? "storage/sd1" : "storage/sd0", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, 0);
        if (!mt) {
            printf("mount %s fail\n", sd_name);
            err = -EFAULT;
            goto __err;
...
int sdcard_storage_device_format(const char *sd_name)
{
    ...
    err = f_format(sd_name, FAT_FILE_SYSTEM_NAME, 32 * 1024, 0);
    if (err == 0) {
        mount_sd_to_fs(sd_name);
    }
...
static int mount_udisk_to_fs(int id)
{
    ...
    mt = mount(name, id ? "storage/udisk1" : "storage/udisk0", FAT_FILE_SYSTEM_NAME, FAT_CACHE_NUM, NULL, 0);
    if (!mt) {
        printf("mount %s fail\n", name);
        err = -EFAULT;
    } else {
...
int udisk_storage_device_format(int id)
{
    ...
    err = f_format(id ? "udisk1" : "udisk0", FAT_FILE_SYSTEM_NAME, 32 * 1024, 0);
    if (err == 0) {
        mount_udisk_to_fs(id);
    }
