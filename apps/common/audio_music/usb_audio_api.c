...
/* #define CONFIG_USB_AUDIO_SYNC_ENABLE */   //软件同步采样率
#define CONFIG_USB_AUDIO_SYNC_BY_HW_SRC_ENABLE  //硬件src同步采样率
...
struct usb_slave_spk_handle {
    ...
    u32 sync_buff_size;
#if defined CONFIG_USB_AUDIO_SYNC_ENABLE
    u8 *dacsyn_ptr;
    dac_usb_syn_ops *dacUSBsyn_ops;
#endif
#if defined CONFIG_USB_AUDIO_SYNC_BY_HW_SRC_ENABLE
    u8 inc_step;
    u8 dec_step;
    s16 adjust_step;
    s16 max_step;
...
#else
    if (hdl && hdl->play_start && hdl->sample_sync_buf) {
        if (0 == cbuf_write(&hdl->sample_sync_cbuf, buf, len)) {
            putchar('u');
        }
...
{
#if defined CONFIG_USB_AUDIO_SYNC_ENABLE
    hdl->sync_buff_size = sample_rate * 4 * channel / 1000 * 30;  //11520
#elif defined CONFIG_USB_AUDIO_SYNC_BY_HW_SRC_ENABLE
    hdl->sync_buff_size = sample_rate * 4 * channel / 1000 * 30;
#else
    hdl->sync_buff_size = 640 * 2;
#endif
    hdl->sample_sync_buf = zalloc(hdl->sync_buff_size);
    if (!hdl->sample_sync_buf) {
        return -ENOMEM;
    }

    os_sem_create(&hdl->rd_sem, 0);
    cbuf_init(&hdl->sample_sync_cbuf, hdl->sample_sync_buf, hdl->sync_buff_size);
    ...
    hdl->dacUSBsyn_ops->open(hdl->dacsyn_ptr, hdl->sync_buff_size / 2, &sf);
#endif
...
#if defined CONFIG_USB_AUDIO_SYNC_BY_HW_SRC_ENABLE
static int uac_sync_resample(void *priv, u32 data_size, u16 *in_rate, u16 *out_rate)
{
    struct usb_slave_spk_handle *hdl = (struct usb_slave_spk_handle *)priv;

    data_size = cbuf_get_data_size(&hdl->sample_sync_cbuf);

    int sample_rate = 48000;
    if (data_size >= (hdl->sync_buff_size * 3 / 5)) {
        sample_rate += (sample_rate * 20 / 10000);  //96
    }
    if (data_size <= (hdl->sync_buff_size * 2 / 5)) {
...
static int usb_audio_speaker_open(struct usb_slave_spk_handle *hdl, const usb_dev usb_id, int value)
{
    ...
    req.dec.dec_sync        = uac_sync_resample;
