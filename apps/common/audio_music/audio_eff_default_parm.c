...
#define LOG_TAG_CONST  AUDIO_EFFECT
...
struct music_parm_tool_set music_mode[mode_add];	//fl eq & drc

#if defined(TCFG_EQ_SPILT_ENABLE) && TCFG_EQ_SPILT_ENABLE
struct music_eq_tool fr_eq_parm[mode_add];	//fr eq
#endif

struct audio_eq *music_eq_front_open(u32 sample_rate, u8 out_32bit_enable)
{
    ...
    u8 tar = nor_label;
    param.seg			= music_mode[tar].eq_parm.seg;
    param.global_gain	= music_mode[tar].eq_parm.global_gain;
#if defined(TCFG_EQ_SPILT_ENABLE) && TCFG_EQ_SPILT_ENABLE
    ...
    param.seg_r			= fr_eq_parm[tar].seg;
    param.global_gain_r	= fr_eq_parm[tar].global_gain;
#endif
    param.max_nsection	= music_mode[tar].eq_parm.seg_num;
    param.nsection		= music_mode[tar].eq_parm.seg_num;
    param.out_32bit		= out_32bit_enable;

    struct audio_eq *eq = audio_dec_eq_open(&param);
    ASSERT(eq);
    return eq;
}
...
#if defined(TCFG_EQ_SPILT_ENABLE)&&TCFG_EQ_SPILT_ENABLE
struct music_eq_tool rr_eq_parm[mode_add];	//rr eq
#endif

struct audio_eq *music_eq_rear_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_eq_param param = {0};
    param.channels 	= 2;
    param.sr 		= sample_rate;
    param.cb		= eq_get_filter_info;
    param.eq_name	= AEID_MUSIC_RL_EQ;
    u8 tar = nor_label;
    param.seg			= rl_eq_parm[tar].seg;
    param.global_gain	= rl_eq_parm[tar].global_gain;
#if defined(TCFG_EQ_SPILT_ENABLE) && TCFG_EQ_SPILT_ENABLE
    ...
    param.seg_r			= rr_eq_parm[tar].seg;
    param.global_gain_r	= rr_eq_parm[tar].global_gain;
#endif
    param.max_nsection	= rl_eq_parm[tar].seg_num;
    param.nsection		= rl_eq_parm[tar].seg_num;
    param.out_32bit		= out_32bit_enable;
    struct audio_eq *eq = audio_dec_eq_open(&param);
    ASSERT(eq);
    return eq;
...
struct audio_drc *music_drc_fl_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_drc_param drc_param = {0};
    drc_param.sr 		= sample_rate;
    drc_param.channels 	= 2;
    drc_param.drc_name 	= AEID_MUSIC_DRC;
    drc_param.cb 		= drc_get_filter_info;
    u8 tar = nor_label;
#if (defined(TCFG_AUDIO_MDRC_ENABLE) && TCFG_AUDIO_MDRC_ENABLE != 0)
    drc_param.crossover	= &music_mode[tar].drc_parm.crossover;
    drc_param.nband		= CROSSOVER_EN
#if (TCFG_AUDIO_MDRC_ENABLE == 2)
                          | MORE_BAND_EN
#endif
                          | 0;
#endif
    drc_param.wdrc		= music_mode[tar].drc_parm.wdrc_parm;	//drc
    drc_param.out_32bit = out_32bit_enable;
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    drc_param.spilt			= 1;
    drc_param.start_point	= 0;
#endif
...
struct audio_drc *music_drc_fr_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_drc_param drc_param = {0};
    drc_param.sr 		= sample_rate;
    drc_param.channels 	= 2;
    drc_param.drc_name 	= AEID_MUSIC_FR_DRC;
    drc_param.cb 		= drc_get_filter_info;
    u8 tar = nor_label;
    drc_param.wdrc		= &fr_drc_parm[tar].wdrc_parm[0];
    drc_param.out_32bit = out_32bit_enable;
    drc_param.spilt			= 1;
    drc_param.start_point	= 1;
...
struct nband_drc rl_drc_parm[mode_add];//rl drc

struct audio_drc *music_drc_rl_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_drc_param drc_param = {0};
    drc_param.sr 		= sample_rate;
    drc_param.channels 	= 2;
    drc_param.drc_name 	= AEID_MUSIC_RL_DRC;
    drc_param.cb 		= drc_get_filter_info;
    u8 tar = nor_label;
    drc_param.wdrc		= &rl_drc_parm[tar].wdrc_parm[0];
    drc_param.out_32bit = out_32bit_enable;
#if (defined(TCFG_DRC_SPILT_ENABLE) && TCFG_DRC_SPILT_ENABLE)
    drc_param.spilt			= 1;
    drc_param.start_point	= 0;
...
struct audio_drc *music_drc_rr_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_drc_param drc_param = {0};
    drc_param.sr 		= sample_rate;
    drc_param.channels 	= 2;
    drc_param.drc_name 	= AEID_MUSIC_RR_DRC;
    drc_param.cb 		= drc_get_filter_info;
    u8 tar = nor_label;
    drc_param.wdrc		= &rr_drc_parm[tar].wdrc_parm[0];
    drc_param.out_32bit = out_32bit_enable;
    drc_param.spilt			= 1;
    drc_param.start_point	= 1;
...
#if defined(TCFG_LAST_WHOLE_DRC_ENABLE) && TCFG_LAST_WHOLE_DRC_ENABLE

wdrc_struct_TOOL_SET last_whole_drc_parm[mode_add];

struct audio_drc *music_drc_last_open(u32 sample_rate, u8 out_32bit_enable)
{
    struct audio_drc_param drc_param = {0};
    drc_param.sr 		= sample_rate;
    drc_param.channels 	= 2;
...
static const u16 music_mode_seq[] = {music_mode_seq0, linein_mode_seq};//音乐模式的标号
static const u16 eff_mode_seq[] = {mic_mode_seq0, mic_mode_seq1, mic_mode_seq2, mic_mode_seq3, mic_mode_seq4, mic_mode_seq5, mic_mode_seq6, mic_mode_seq7}; //混响模式标号
