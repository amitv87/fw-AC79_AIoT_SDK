...
static void draw_task_added_event_cb(lv_event_t *e)
{
    ...
    if (((lv_draw_dsc_base_t *)draw_task->draw_dsc)->part != LV_PART_ITEMS) {
        return;
    }
    ...
    if (id < 7) {
        ...
        if (border_draw_dsc) {
            border_draw_dsc->opa = LV_OPA_TRANSP;
        }
    ...
    if (lv_buttonmatrix_has_button_ctrl(obj, id, LV_CALENDAR_CTRL_HIGHLIGHT)) {
        if (border_draw_dsc) {
            border_draw_dsc->color = lv_theme_get_color_primary(obj);
        }
        if (fill_draw_dsc) {
