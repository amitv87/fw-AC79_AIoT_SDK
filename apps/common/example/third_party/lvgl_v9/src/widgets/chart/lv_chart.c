...
void lv_chart_remove_series(lv_obj_t *obj, lv_chart_series_t *series)
{
    ...
    if (!series->x_ext_buf_assigned && series->x_points) {
        lv_free(series->x_points);
    }
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        if (_lv_ll_is_empty(&chart->series_ll) == false) {
            if (chart->type == LV_CHART_TYPE_LINE) {
                ...
                draw_series_bar(obj, layer);
            } else if (chart->type == LV_CHART_TYPE_SCATTER) {
                draw_series_scatter(obj, layer);
            }
        }
...
{
    lv_area_t clip_area;
    if (_lv_area_intersect(&clip_area, &obj->coords, &layer->_clip_area) == false) {
        return;
    }
    ...
    if (LV_MIN(point_w, point_h) > line_dsc.width / 2) {
        line_dsc.raw_end = 1;
    }
    if (line_dsc.width == 1) {
        line_dsc.raw_end = 1;
    ...
    _LV_LL_READ_BACK(&chart->series_ll, ser) {
        ...
        for (i = 0; i < chart->point_cnt; i++) {
            ...
            if (line_dsc.p1.x > clip_area_ori.x2 + point_w + 1) {
                break;
            }
                ...
                if (crowded_mode) {
                    if (ser->y_points[p_prev] != LV_CHART_POINT_NONE && ser->y_points[p_act] != LV_CHART_POINT_NONE) {
                        ...
                        if (line_dsc.p1.x != line_dsc.p2.x) {
                            ...
                            line_dsc.p1.y = y_min;
                            line_dsc.p2.y = y_max;
                            if (line_dsc.p1.y == line_dsc.p2.y) {
                                line_dsc.p2.y++;    /*If they are the same no line will be drawn*/
                            }
                            lv_draw_line(layer, &line_dsc);
...
static void draw_cursors(lv_obj_t *obj, lv_layer_t *layer)
{
    ...
    lv_area_t clip_area;
    if (!_lv_area_intersect(&clip_area, &layer->_clip_area, &obj->coords)) {
        return;
    }
