...
void lv_draw_sw_layer(lv_draw_unit_t *draw_unit, const lv_draw_image_dsc_t *draw_dsc, const lv_area_t *coords)
{
    ...
    lv_area_t draw_area;
    if (!_lv_area_intersect(&draw_area, &area_rot, draw_unit->clip_area)) {
        return;
    }
#endif
                          ...
                          const lv_area_t *img_coords, const lv_area_t *clipped_img_area)
{
    ...
    /*Handle masked RGB565, RGB888, XRGB888, or ARGB8888 images*/
        ...
        if (lv_area_get_width(&draw_dsc->original_area) < 0) {
            original_area = img_coords;
        } else {
            original_area = &draw_dsc->original_area;
        }
    ...
    /* check whether it is possible to accelerate the operation in synchronouse mode */
        ...
        if (transformed) {
            if (cf == LV_COLOR_FORMAT_RGB888 || cf == LV_COLOR_FORMAT_XRGB8888) {
