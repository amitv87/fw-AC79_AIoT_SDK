...
ret_t y_axis_measure_series(widget_t *widget, void *measure_params, object_t *src, object_t *dst)
{
    ...
    if (tk_abs(sinterval) >= 1.0) {
        ...
        bool_t index_of_fold_changed = FALSE;
        uint32_t new_index_of_fold = params->index_of_fold;
        ...
        for (i = 0; i < nr; i++) {
            ...
            if (i == 0) {
                ...
                if (params->draw_data_compare_series(d, dmin) == 0 && index + 1 < src_size) {
                    ...
                    uint32_t dst_size = SERIES_FIFO_GET_SIZE(dst);

                    if (params->draw_data_compare_value(dmin, dmax) == 0) {
                        break_if_true(dst_size + 1 > nr);
                        series_fifo_push(dst, dmin);

                        if (!index_of_fold_changed && i >= params->index_of_fold) {
                            index_of_fold_changed = TRUE;
                            new_index_of_fold = dst_size + (i == params->index_of_fold ? 1 : 0);
                        }
                    } else {
                        break_if_true(dst_size + 2 > nr);
                        if (increase) {
                        ...
                        if (!index_of_fold_changed && i >= params->index_of_fold) {
                            index_of_fold_changed = TRUE;
                            new_index_of_fold = dst_size + (i == params->index_of_fold ? 2 : 0);
                        }
                    }
        ...
        params->index_of_fold = new_index_of_fold;
