...
{
    return tk_roundi(((series_data_draw_normal_t *)a)->y) -
                                        ...
                                        uint32_t *new_index_of_fold)
{
    ...
    ret = axis_measure_series(vaxis, (void *)(&params), series->fifo, fifo);
    if (ret == RET_OK && new_index_of_fold != NULL) {
        *new_index_of_fold = params.index_of_fold;
    }
    return ret;
}
...
ret_t series_p_on_paint_self_push(widget_t *widget, canvas_t *c)
{
    ...
    series_p_measure_draw_data(series, saxis, vaxis, fifo, 0, vertical, NULL);
...
ret_t series_p_on_paint_self_cover(widget_t *widget, canvas_t *c)
{
    ...
    series_p_measure_draw_data(series, saxis, vaxis, fifo, index_of_fold, vertical, &index_of_fold);
    series_p_get_origin_point(widget, saxis, vaxis, FALSE, &o);
