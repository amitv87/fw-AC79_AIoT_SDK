...
NVGpaint nvgImagePatternWithXform(NVGcontext *ctx, float *xform, float w, float h, int image, float alpha)
{
    NVGpaint p;
    NVG_NOTUSED(ctx);
    memset(&p, 0, sizeof(p));
