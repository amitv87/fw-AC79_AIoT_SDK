...
int bp__page_read(bp_db_t *t, bp__page_t *page)
{
    ...
    while (o < size) {
        ...
        memcpy(&tmp1, buff + o, 8);
        memcpy(&tmp2, buff + o + 8, 8);
        memcpy(&tmp3, buff + o + 16, 8);
        page->keys[i].length = ntohll((tmp1));
        page->keys[i].offset = ntohll((tmp2));
        page->keys[i].config = ntohll((tmp3));
    ...
    page->length = i;
    page->byte_size = size;
    /* printf("bp__page_read  page->length:%d  page->byte_size:%d\n",page->length,page->byte_size); */
...
int bp__page_save(bp_db_t *t, bp__page_t *page)
{
    ...
    for (i = 0; i < page->length; i++) {
        /* put_buf(&page->byte_size,sizeof(page->byte_size)); */
        /* put_buf(&page->keys[i],sizeof(page->keys[i])); */
        /* printf("o:%d  %d  %d\n",o, BP__KV_SIZE(page->keys[i]),page->byte_size); */
        ...
        */
        ...
        uint64_t  tmp1 = htonll(page->keys[i].length);
        uint64_t  tmp2 = htonll(page->keys[i].offset);
        uint64_t  tmp3 = htonll(page->keys[i].config);

        memcpy(buff + o, &tmp1, 8);
        memcpy(buff + o + 8, &tmp2, 8);
        memcpy(buff + o + 16, &tmp3, 8);
                        ...
                        void *arg)
{
    ...
    /* printf("index:%d\n",index); */
    /* put_buf(&page->keys[index],sizeof(page->keys[index])); */
    /* put_buf(&page->keys[index+1],sizeof(page->keys[index+1])); */
                       ...
                       void *arg)
{
    ...
    for (i = start_res.index; i <= end_res.index; i++) {
        ...
        if (page->type == kPage) {
            ...
            if (cb(arg, (bp_key_t *) &page->keys[i], &value) < 0) {
