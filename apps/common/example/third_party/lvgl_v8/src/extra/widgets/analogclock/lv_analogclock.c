...
{
    lv_analogclock_t *analogclock = (lv_analogclock_t *)obj;
    lv_analogclock_dial_t *dial = _lv_ll_ins_head(&analogclock->dial_ll);
    LV_ASSERT_MALLOC(dial);
    lv_memset_00(dial, sizeof(lv_analogclock_dial_t));
    dial->opa = LV_OPA_MAX;
    dial->src = src;
    analogclock->dial = dial;
}
...
static void lv_analogclock_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    _lv_ll_init(&analogclock->dial_ll, sizeof(lv_analogclock_dial_t));
...
static void lv_analogclock_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    _lv_ll_clear(&analogclock->dial_ll);
...
static void draw_ticks_and_labels(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
    ...
    if (analogclock->dial != NULL) {
        ...
        part_draw_dsc.class_p = MY_CLASS;
        part_draw_dsc.p1 = &scale_center;
        const void *src = analogclock->dial->src;
        lv_img_header_t header;
        lv_area_t coords_part;
        lv_img_decoder_get_info(src, &header);
        lv_coord_t center_x = scale_area->x1 + (scale_area->x2 - scale_area->x1) / 2;
        lv_coord_t center_y = scale_area->y1 + (scale_area->y2 - scale_area->y1) / 2;
        coords_part.x1 = center_x - (header.w / 2);
        coords_part.y1 = center_y - (header.h / 2);
        coords_part.x2 = center_x - (header.w / 2) + header.w - 1;
        coords_part.y2 = center_y - (header.h / 2) + header.h - 1;
        lv_opa_t opa = lv_obj_get_style_bg_opa(obj, LV_PART_MAIN);
        img_dsc.opa = opa;
        lv_event_send(obj, LV_EVENT_DRAW_MAIN_BEGIN, &part_draw_dsc);
    ...
    _LV_LL_READ_BACK(&analogclock->scale_ll, scale) {
        ...
        for (i = 0; i < scale->tick_cnt; i++) {
            ...
            lv_analogclock_indicator_t *indic;
            _LV_LL_READ_BACK(&analogclock->indicator_ll, indic) {
                if (indic->type != LV_analogclock_INDICATOR_TYPE_SCALE_LINES) {
                    continue;
                }
