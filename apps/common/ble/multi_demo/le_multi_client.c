    ...
    if (rw_flag == 0) {
        ...
        if (!info) {
            ...
            for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
                ...
                /* && 0 == memcmp(&info->conn_interval, &conn_pair_info_table[i].conn_interval, 6)) { */
                /* log_info("dev in table\n"); */
                /* conn_pair_info_table[i].conn_handle = info->conn_handle;//update table's handle,not update to vm */
                /* return; */
                /* } */
