...
static u8 FCC_BT_DUT_ARRAY[]    = {0x01, 0xA1, 0xA2, 0x06, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
static u8 FCC_BT_BQB_ARRAY[]    = {0x01, 0xA1, 0xA2, 0x06, 0x01, 0x02, 0x03, 0x04, 0x05, 0x07};
...
#ifdef CONFIG_BT_ENABLE
        rf_fcc_save_change_mode(FCC_BT_DUT_MODE);
        *reset = 1;
        *rsp_len = sizeof(FCC_RSP_ARRAY);
        return FCC_RSP_ARRAY;
#else
...
#ifdef CONFIG_BT_ENABLE
        rf_fcc_save_change_mode(FCC_BT_BQB_MODE);
        *reset = 1;
        *rsp_len = sizeof(FCC_RSP_ARRAY);
        return FCC_RSP_ARRAY;
#else
