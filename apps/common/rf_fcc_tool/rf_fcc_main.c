...
static u8 comm_dev_init(void)
{
    ...
    dev_ioctl(__THIS->uart_hdl, UART_SET_CIRCULAR_BUFF_ADDR, (u32)__THIS->uart_circlebuf);
    dev_ioctl(__THIS->uart_hdl, UART_SET_CIRCULAR_BUFF_LENTH, sizeof(__THIS->uart_circlebuf));
    dev_ioctl(__THIS->uart_hdl, UART_SET_RECV_BLOCK, 1);
    dev_ioctl(__THIS->uart_hdl, UART_SET_RECV_TIMEOUT, 1000);
    dev_ioctl(__THIS->uart_hdl, UART_START, 0);
#endif
    return 0;
}
