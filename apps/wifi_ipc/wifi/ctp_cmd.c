...
int cmd_put_format(void *priv, char *content)
{
    ...
    if (storage_device_ready()) {
        err = f_format(CONFIG_ROOT_PATH, "fat", 32 * 1024, 0);
        ...
        err = f_format(SDX_DEV, "fat", 32 * 1024, 0);
...
static int cmd_put_net_ota(void *priv, char *content)
{
    ...
    json_object_put(new_obj);
    val = !!val;
#if 0 //(CONFIG_DOUBLE_BANK_ENABLE == 0)
    if (val) {
...
int cmd_put_open_speak_stream(void *priv, char *content)
{
    printf("cmd_put_open_speak_stream\n");
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    char buf[256];
    int ret = 0;
    const char *format = NULL, *rate = NULL;
    new_obj = json_tokener_parse(content);
    parm =  json_object_object_get(new_obj, "param");
    format = json_object_get_string(json_object_object_get(parm, "format"));
    rate = json_object_get_string(json_object_object_get(parm, "rate"));
    ...
    if (format && atoi(format) == 1) {
        format1  = 1;
    } else if (format && atoi(format) == 0) {

        format1 = 0;
    } else {

        format1 = 0;
        printf("undefined info.type ,default use pcm\n");
    }


    printf("rate:%d  format:%d\n", rate1, format1);

    extern int open_audio_channel(void *arg);
    ret = open_audio_channel(priv);

    if (ret < 0) {
        CTP_CMD_COMBINED(priv, CTP_RT_OPEN_FAIL, "OPEN_SPEAK_STREAM", "NOTIFY", CTP_RT_OPEN_FAIL_MSG);
    } else {

        int len = sprintf(buf, "format:%d,rate:%d", format1, rate1);

        printf("buf=>%s len:%d\n", buf, len);
        CTP_CMD_COMBINED(priv, CTP_NO_ERR, "OPEN_SPEAK_STREAM", "NOTIFY", buf);
...
static int cmd_put_close_speak_stream(void *priv, char *content)
{
    json_object *new_obj = NULL;
    json_object *parm = NULL;
    char buf[128];
    ...
    if (atoi(status)) {
        extern int close_audio_channel(void *arg);
        ret = close_audio_channel(priv);
        if (ret < 0) {
            CTP_CMD_COMBINED(priv, CTP_REQUEST, "CLOSE_SPEAK_STREAM", "NOTIFY", CTP_REQUEST_MSG);
        } else {

            sprintf(buf, "status:%d", 1);
            CTP_CMD_COMBINED(priv, CTP_NO_ERR, "CLOSE_SPEAK_STREAM", "NOTIFY", buf);

        }

    }

    printf("cmd_put_close_speak_stream\n");
...
const struct ctp_map_entry ctp_video_cmd_tab[] SEC_USED(.ctp_video_cmd) = {
    ...
    {NULL, "OPEN_SPEAK_STREAM", NULL, cmd_put_open_speak_stream},
    {NULL, "CLOSE_SPEAK_STREAM", NULL, cmd_put_close_speak_stream},
