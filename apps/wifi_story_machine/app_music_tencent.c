...
#define WIFI_BT_SEPARATE	1	//WIFI蓝牙独立运行
...
extern u8 tts_start_play_flag;		//用与tts与播歌互斥
...
static int __net_download_ready()
{
    ...
    if (__this->download_ready && !tts_start_play_flag) {
        return 1;
    }
    return 0;
}
...
static int net_music_dec_file(void *_url, int breakpoint, void *handler, int arg)
{
    ...
    /* parm.start_play_threshold = 1024 * 40; */
    if (!net_server) {
...
static void dec_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case AUDIO_SERVER_EVENT_END:
        ...
        if (__this->bt_connecting && !__this->bt_emitter_enable) {
            if (get_total_connect_dev() == 1 && get_bt_music_dec_ops()->dec_status(0) != AUDIO_DEC_START) {
...
static int play_mode_switch(void)
{
    if (__this->local_play_mode == LOOP_PLAY_MODE) {
        __this->local_play_mode = SINGLE_PLAY_MODE;
    } else if (__this->local_play_mode == SINGLE_PLAY_MODE) {
        __this->local_play_mode = RANDOM_PLAY_MODE;
    } else {
        __this->local_play_mode = LOOP_PLAY_MODE;
    }
    printf("__this->local_mode = %d\n\r", __this->local_play_mode);
    return 0;
}
...
static int app_music_key_click(struct key_event *key)
{
    ...
    case KEY_POWER:
        ...
        break;
    case KEY_PHOTO:
        if (__this->bt_music_enable && __this->dec_ops) {
            __this->dec_ops->dec_progress(NULL);
        }
        break;
    default:
        break;
