...
const struct wifi_calibration_param wifi_calibration_param = {
    .xosc_l     = 0xb,// 调节左晶振电容
    .xosc_r     = 0xa,// 调节右晶振电容
    .pa_trim_data  ={1, 5, 6, 6, 9, 0},// 根据MP测试生成PA TRIM值
	.mcs_dgain     ={
		55,//11B_1M
	  	55,//11B_2.2M
	  	55,//11B_5.5M
...
{
    //norflash_ioctl(NULL, IOCTL_SET_SPI_ATTR, spi0_data.attr);

    extern void dac_early_init(u8 trim_en, u8 pwr_sel, u32 dly_msecs);
    dac_early_init(1, dac_data.differ_output ? (dac_data.ch_num > 1 ? 0xf : 0x3): dac_data.hw_channel, 1000);
    devices_init();
}
