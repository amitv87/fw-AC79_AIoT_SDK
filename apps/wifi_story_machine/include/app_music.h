...
#define BREAKPOINT_DATA_LEN 2040
#elif defined CONFIG_FLAC_DEC_ENABLE
#define BREAKPOINT_DATA_LEN	688
#elif defined CONFIG_M4A_DEC_ENABLE
...
struct local_breakpoint {
#ifdef CONFIG_RESUME_LOCAL_PLAY_FILE
    u32 sclust;
    u32 fsize;
    u8 data[BREAKPOINT_DATA_LEN];
#endif
...
enum {
    ...
    SDCARD0_MUSIC_MODE, //sd0播放
    SDCARD1_MUSIC_MODE, //sd1播放
    UDISK0_MUSIC_MODE,  //usb1.1-u盘播放
    UDISK1_MUSIC_MODE,  //usb2.0-u盘播放
    AUX_MUSIC_MODE,     //LINEIN播放模式
    FM_MUSIC_MODE,      //FM电台播放
    SDCARD_MUSIC_MODE = SDCARD0_MUSIC_MODE, //兼容旧版sd卡播放
    UDISK_MUSIC_MODE = UDISK0_MUSIC_MODE,   //兼容旧版u盘播放
};
...
typedef enum {
    ...
    /*64*/    APP_LOCAL_PROMPT_ENTER_AUX_MUSIC_MODE,   //进入AUX音乐播放模式
    /*65*/    APP_LOCAL_PROMPT_FORMAT_NOT_SUPPORT,     //播放器不支持的媒体格式
    /*66*/    APP_LOCAL_PROMPT_SEEK_UNSUPPORTED,       //不支持快进快退的本地提示音
    /*67*/    APP_LOCAL_PROMPT_ENTER_BT_EMITTER_MODE,  //进入蓝牙发射器模式
    /*68*/    APP_LOCAL_PROMPT_EXIT_BT_EMITTER_MODE,   //退出蓝牙发射器模式
...
struct app_music_hdl {
    ...
    u16 wait_udisk;
    u16 wait_udisk1;
    u16 udisk_upgrade_timer;
    u16 udisk1_upgrade_timer;
    int seek_step;
    int play_time;
    int save_play_time;
    int total_time;
    ...
    u8 sdram_prompt_dir;
    u8 sdram_prompt_flag;
    u8 last_sdram_prompt_flag;
    u8 sdram_save_dec_type[4];
#endif
};
