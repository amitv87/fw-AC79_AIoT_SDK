...
static void bt_wait_phone_connect_control(u8 enable)
{
    ...
    if (enable) {
        log_info("is_1t2_connection:%d \t total_conn_dev:%d", is_1t2_connection(), get_total_connect_dev());
...
static void sync_call_volume_timer(void *p)
{
    ...
    user_send_cmd_prepare(USER_CTRL_HFP_CALL_SET_VOLUME, 1, &call_volume);
}
...
static void a2dp_drop_frame(void *p)
{
    ...
    if (num > 1) {
        for (int i = 0; i < (num - 1); i++) {
            ...
            /* log_info("a2dp_drop_frame: %d", len); */
...
static void a2dp_audio_codec_open(void)
{
    ...
    if (!__this->dec_server) {
        __this->dec_server = earphone_a2dp_audio_codec_open(__this->media_type, __this->media_volume);//A2DP_CODEC_SBC);
        if (__this->dec_server) {
...
{
    return __this->a2dp_drop_frame_timer;
}
...
{
    if (is_1t2_connection() || get_total_connect_dev() > 0) {
        user_send_cmd_prepare(USER_CTRL_PAGE_CANCEL, 0, NULL);
...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
...
#elif CONFIG_POWER_ON_ENABLE_EMITTER
#if TCFG_BT_SNIFF_ENABLE
    ...
    case BT_STATUS_PHONE_INCOME:
        ...
        if ((check_esco_state_via_addr(tmp_bd_addr) != BD_ESCO_BUSY_OTHER) && (__this->phone_ring_flag == 0)) {
            ...
            log_info("SCO busy now:%d,%d", check_esco_state_via_addr(tmp_bd_addr), __this->phone_ring_flag);
        }
        break;
    ...
    case BT_STATUS_A2DP_MEDIA_STOP:
        ...
        __this->media_play_flag = 0;
#if (TCFG_BD_NUM == 2)
        extern void free_a2dp_using_decoder_conn(void);
...
#if TCFG_USER_EMITTER_ENABLE
        if (BT_EMITTER_EN == bt_emitter_role_get() && (get_emitter_curr_channel_state() & A2DP_SRC_CH) == 0) {
...
static void emitter_auto_connection_deal(void *arg)
{
    if (++__this->emitter_reconnect_cnt > 5) {
        ...
        __this->emitter_reconnect_cnt = 0;
        user_send_cmd_prepare(USER_CTRL_WRITE_CONN_ENABLE, 0, NULL);
#if BT_SUPPORT_EMITTER_PAGE_SCAN
        user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_ENABLE, 0, NULL);
#endif
        return;
    }
