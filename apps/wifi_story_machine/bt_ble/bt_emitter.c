...
void emitter_rx_vol_change(u8 vol)
{
    ...
    int volume = vol * 100 / 127;

    if (get_app_music_volume() > volume) {
        key.action = KEY_EVENT_CLICK;
        key.value = KEY_VOLUME_DEC;
        key.type = KEY_EVENT_USER;
        key_event_notify(KEY_EVENT_FROM_USER, &key);
    } else if (get_app_music_volume() <= volume && volume != 0) {
        key.action = KEY_EVENT_CLICK;
        key.value = KEY_VOLUME_INC;
        key.type = KEY_EVENT_USER;
        key_event_notify(KEY_EVENT_FROM_USER, &key);
...
u8 bt_emitter_pp(u8 pp)
{
    ...
    if ((get_emitter_curr_channel_state() & (A2DP_SRC_CH | A2DP_CH)) != (A2DP_SRC_CH | A2DP_CH)) {
        return 0;
    }
...
void emitter_get_vm_device()
{
    ...
    u8 flag = restore_remote_device_info_opt(remote_addr, 10, get_remote_dev_info_index());
    for (u8 i = 0; i < flag; i++) {
        log_d("----- \n");
...
void bt_emitter_esco_pp(u8 pp)
{
    ...
    if (pp) {
        ...
        if (get_call_status() == BT_CALL_INCOMING) {
            bt_emitter_pp(0);
            user_emitter_cmd_prepare(USER_CTRL_HFP_CALL_ANSWER, 0, NULL);
        } else if (get_call_status() != BT_CALL_HANGUP) {
