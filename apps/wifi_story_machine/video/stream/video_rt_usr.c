...
static void get_one_frame(u8 *inputFrame, u32 len, int width, int height)
{
    ...
    if (is_in_config_network_state()) {
        if (qr_decoder && qr_flag) {
            ...
            extern int YUV420p_Cut(unsigned char *yuv420p, int src_w, int src_h, unsigned char *out, int out_size, int w_start, int w_size, int h_start, int h_size);
#if 1
            YUV420p_Soft_Scaling(inputFrame, inputFrame, width, height, width / 4, height / 3);
#else
            YUV420p_Cut(inputFrame, width, height, inputFrame, height * width * 3 / 2, (width -  width / 4) / 2, width / 4, (height - height / 3) / 2, height / 3);
#endif
#else
