...
#if CONFIG_POWER_ON_ENABLE_BT || CONFIG_POWER_ON_ENABLE_BLE
...
static int compare_dhcp_ipaddr(void)
{
    ...
    if (sta_channel == sta_ip_info.chanel
        && !strcmp(info.ssid, sta_ip_info.ssid)
...
#endif
        wifi_rxfilter_cfg(3); //过滤not_my_bssid
        break;
...
static void wifi_app_task(void *priv)  //主要是create wifi 线程的
{
    ...
    sys_timeout_add_to_task("app_core", NULL, switch_rf_coexistence_timer, 15000);
#endif
#endif
}
