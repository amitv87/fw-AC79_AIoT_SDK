...
static void test_main(u8 *buf, u32 len, int yuv_in_w, int yuv_in_h)
{
    ...
    if (get_lcd_turn_mode() == 0) {
        YUV420p_Soft_Scaling(buf, NULL, yuv_in_w, yuv_in_h, LCD_H, LCD_W);
    } else {
        YUV420p_Soft_Scaling(buf, NULL, yuv_in_w, yuv_in_h, LCD_W, LCD_H);
    }
...
static void camera_show_lcd(u8 *buf, u32 size, int width, int height)
{
    ...
    printf(">>>>>>>>>>>>>>>sssid = %s, pwd = %s", ssid, pwd);
#endif
    YUV420p_Soft_Scaling(buf, NULL, width, height, LCD_H, LCD_W);
