...
u8 net_isc_sbuf[NET_ISC_SBUF_SIZE] SEC(.sram) ALIGNE(32);//必须32对齐
#endif
...
int net_video1_open(void *net_priv)
{
    ...
    */
    ...
    if (prodct_jpeg_test_get()) { //自动量产测试模式
        sprintf(req.rec.net_par.netpath, "usr://192.168.1.1:8000");
    }
#endif
...
static int net_video_rec_take_photo(void (*callback)(char *buffer, int len))
{
    ...
    server = (__this && __this->state == VIDREC_STA_START) ? __this->video_rec0 : __this_net->net_video_rec;
    req.icap.quality = VIDEO_MID_Q;
    req.icap.buf_size = CAP_IMG_SIZE;
#ifdef CONFIG_VIDEO_SPEC_DOUBLE_REC_MODE
    req.icap.width = 1280;//拍照固定分辨率
    req.icap.height = 720;//拍照固定分辨率
    if (req.icap.width >= 1280 && req.icap.buf_size < 120 * 1024) {//大分辨率大内存JPEG
        req.icap.buf_size = 120 * 1024;
    }
#endif
    req.icap.buf = malloc(req.icap.buf_size);
    if (!req.icap.buf) {
        goto error;
    }
...
{
#ifdef CONFIG_VIDEO_SPEC_DOUBLE_REC_MODE
    return (__SDRAM_SIZE__ > 2 * 1024 * 1024) ? 0 : 200 * 1024;
#endif
    return 0;
}
