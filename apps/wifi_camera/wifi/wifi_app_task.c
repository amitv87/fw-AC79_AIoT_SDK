...
static int wifi_event_callback(void *network_ctx, enum WIFI_EVENT event)
{
    ...
    case WIFI_EVENT_MODULE_INIT:
        ...
        wifi_set_default_mode(&wifi_default_mode_parm, 0, wifi_default_mode_parm.mode == STA_MODE || !CONFIG_DOUBLE_BANK_ENABLE); //配置STA模式情况下,把默>    认配置SSID也存储起来,以后即使保存过其他SSID,也不会覆盖丢失,使用连接最优信号SSID策略的情况下可以匹配连接
        break;
