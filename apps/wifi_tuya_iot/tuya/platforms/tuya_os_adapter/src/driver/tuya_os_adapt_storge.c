...
/** 涂鸦分区路径*/
#define TUYA_DATA_AREA_PATH "mnt/sdfile/EXT_RESERVED/tuya"
/***********************************************************
*************************variable define********************
***********************************************************/
...
int tuya_os_adapt_flash_read(const unsigned int addr, unsigned char *dst, const unsigned int size)
{
    ...
    sdfile_reserve_zone_read(dst, addr, size, 0);
...
int tuya_os_adapt_flash_write(const unsigned int addr, const unsigned char *src, const unsigned int size)
{
    ...
    sdfile_reserve_zone_write(src, addr, size, 0);
...
#define FLASH_BLOCK 4096
    ...
    if (length >= size) {
        sdfile_reserve_zone_erase(beginBlock * FLASH_BLOCK, SDFILE_SECTOR_SIZE, 0);//每个块，写之前擦除对应块
        return OPRT_OS_ADAPTER_OK;
    } else {
        sdfile_reserve_zone_erase(beginBlock * FLASH_BLOCK, SDFILE_SECTOR_SIZE, 0);//每个块，写之前擦除对应块
        writeLength = length;
    ...
    for (int i = beginBlock + 1; i < endBlock; i++) {
        sdfile_reserve_zone_erase(i * FLASH_BLOCK, SDFILE_SECTOR_SIZE, 0);//每个块，写之前擦除对应块
        writeLength += FLASH_BLOCK;
    }

    if (beginBlock != endBlock && (size > writeLength)) {
        sdfile_reserve_zone_erase(endBlock * FLASH_BLOCK, SDFILE_SECTOR_SIZE, 0);//每个块，写之前擦除对应块
    }
...
int tuya_os_adapt_legacy_swap_get_desc(unsigned int *addr, unsigned int *size)
{
    ...
    unsigned int uf_addr = file_attr.sclust;
