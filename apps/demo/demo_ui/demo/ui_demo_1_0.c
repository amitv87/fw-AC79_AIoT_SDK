...
{
    image_mub = !image_mub;
    ui_pic_show_image_by_id(BASEFORM_18, image_mub);
...
{
    start_animation_show(PAGE_1, BASEFORM_2, 11, 14); //320*240 最快14fps
}
#endif
...
static void ui_demo_test(void *priv)
{
    ...
    ui_show_main(PAGE_3);
    for (u8 i = 0; i < 100; i++) {
        ui_slider_set_persent_by_id(NEWLAYOUT_21, i);
        os_time_dly(10);
    }
    ui_hide_curr_main(); //关闭上一个画面
    ...
    ui_text_show_index_by_id(BASEFORM_11, language_indx);//切换语言文字
    os_time_dly(DELAY_TIME);
    ...
    while (1) {
        for (u8 i = 0; i < 11; i++) {
            ui_pic_show_image_by_id(BASEFORM_2, i);//这里需要注意图层背景不能设置为无色 不然会显示不了动态图
            log_info("idx =%d.\n", i);
        }
    }
}
