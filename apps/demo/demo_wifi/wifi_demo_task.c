...
static void wifi_status(void *p)
{
    if (wifi_is_on()) {
        ...
        if (info.mode == AP_MODE) {
            ...
            printf("Router_RSSI=%d \r\n", wifi_get_rssi()); //侦测路由器端信号质量
        }
    }
}
...
static void wifi_scan_test(void)
{
    ...
    os_time_dly(8 * 100); //简单等待一些时间, 或者通过信号量/标志位 等待事件 WIFI_STA_SCAN_COMPLETED 扫描完成之后才去获取结果
    ...
    for (int i = 0; i < sta_ssid_num; i++) {
        printf("wifi_sta_scan_test ssid = [%s]\r\n", sta_ssid_info[i].ssid);
    }
