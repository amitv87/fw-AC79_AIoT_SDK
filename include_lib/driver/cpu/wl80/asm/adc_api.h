...
void adc_pmu_detect_en(u32 ch);//使能adc通道

void adc_vdc14_save(void);//存储vdc14的值

void adc_vdc14_restore(void);//再次存储vdc14的值，避免被中途修改

u32 adc_get_value(u32 ch);//获取adc通道测得的数值(3.3*数值/1024)即得对应的电压值

u32 adc_add_sample_ch(u32 ch);//添加adc测试通道

u32 adc_remove_sample_ch(u32 ch);//移除adc测试通道

u32 adc_get_voltage(u32 ch);//获取adc通道电压值，如果测得与实际不符，则需留意芯片是否trim过，trim值是否正确。
