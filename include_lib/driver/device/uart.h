...
struct uart_platform_data {
    ...
    int rts_pin;				/*!<  串口一控制流RTS脚*/
    int cts_pin;				/*!<  串口一控制流CTS脚*/
    int rts_output_channel;		/*!<  RTS输出通道,只支持输出通道2，3*/
...
#define UART1_FLOW_CTL_RTS_SUSPEND          _IO(UART_MAGIC,11)					/*!<  串口1控制流挂起*/
#define UART1_FLOW_CTL_RTS_RESUME           _IO(UART_MAGIC,12)					/*!<  串口1控制流恢复*/
#define UART_SET_9BIT_MODE                  _IOW(UART_MAGIC,13,bool)			/*!<  设置串口接收是否使用9bit模式*/
/* \} name */
