...
local fix_avol_cfg = cfg:i32("固定模拟音量", 0);
local fix_avol_cfg_view = cfg:checkBoxView(fix_avol_cfg);

local find_nearest_vol_from_table = function (expect_db, find_db)
	-- 找到第一个比当前值大的数
	local fi = #com_vol_config.table + 1
	for i = 1, #com_vol_config.table do
		if com_vol_config.table[i].rdb < find_db then
			fi = i;
			break
		end
	end
	...
	for i = 1, max_vol_config_count do
		...
		x:setEval(function ()
			...
			if all_com_vol_cfgs[k].com_vol_enable_switch[i].val ~= 0 then
				local fd = all_com_vol_cfgs[k].com_vol_invals[count-i+1].val;
				if fix_avol_cfg.val ~= 0 then
					fd = all_com_vol_cfgs[k].com_vol_invals[1].val;
				end
				local x = find_nearest_vol_from_table(all_com_vol_cfgs[k].com_vol_invals[count-i+1].val, fd);
				return x.x;
			else
				return 0;
			end
...
local output_com_vol_array_header_col2 = function (arrayname, output)
	local outstr = string.format("static unsigned short %s[%d] = {\n",
		arrayname,
		#output + 1);
	outstr = outstr .. string.format("\t%5d, //0: None\n", 0);
	for i = 1, #output do
		local x = output[#output-i+1];
		outstr = outstr .. string.format("\t%5d, // %d:%.2f db\n", x.x, i, x.vol);
	end
	outstr = outstr .. "};\n";
	return outstr;
end
...
local save_button = cfg:stButton("保存头文件", function ()
	...
	local arr_names = {'combined_vol_list', 'call_combined_vol_list', 'digital_vol_list'};
	...
	for k = 1, 3 do
		...
		for i = 1, max_vol_config_count do
			if all_com_vol_cfgs[k].com_vol_enable_switch[i].val ~= 0 then
				local fd = all_com_vol_cfgs[k].com_vol_invals[i].val;
				if fix_avol_cfg.val ~= 0 then
					fd = all_com_vol_cfgs[k].com_vol_invals[1].val;
				end
				local x = find_nearest_vol_from_table(all_com_vol_cfgs[k].com_vol_invals[i].val, fd);
				output[#output+1] = x;
			end
		end
		
		if k == 3 then
			outstr = outstr .. output_com_vol_array_header_col2(arr_names[k], output);
		else
...
comvol_view = {'联合音量', 
	cfg:vBox{
		save_button,
		cfg:hBox {
			cfg:stLabel("固定模拟音量："), fix_avol_cfg_view, cfg:stSpacer(),
		},
		cfg:stTab{
			{"系统音量", all_com_vol_cfgs[1].layout},
			{"通话音量", all_com_vol_cfgs[2].layout},
			{"数字音量", all_com_vol_cfgs[3].layout},
		}
	}
}
