...
local anc_config = {
    ...
    dac_gain = {},      --u8 dac模拟增益 			range 0-15;  default 13
    ref_mic_gain = {},  --u8 参考mic	  			range 0-19;  default 10
    err_mic_gain = {},  --u8 误差mic	  			range 0-19;  default 10
    cmp_en = {},        --u8 音乐补偿使能			range 0-1;   default 1
    drc_en = {},        --u8 DRC使能				range 0-1;   default 0
    ahs_en = {},        --u8 AHS使能				range 0-1;   default 1
    dcc_sel = {},       --u8 DCC选择 				range 0-8;   default 4
...
anc_config.reference_book_view = cfg:stButton("JL_AC700N_ANC调试手册.pdf",
    function()
...
anc_config.version.cfg = cfg:i32("anc_version:  ", 0x7003);
...
-- 8-1 dac增益
anc_config.dac_gain.cfg = cfg:i32("dac_gain:  ", 13);
...
anc_config.dac_gain.hbox_view = cfg:hBox {
    ...
    cfg:stLabel("(dac模拟增益，设置范围: 0 ~ 15，步进：1，默认值：13)"),
    cfg:stSpacer(),
};

-- 8-2 ref_mic_gain
anc_config.ref_mic_gain.cfg = cfg:i32("mic0_gain:  ", 8);
...
-- 8-3 err_mic_gain
anc_config.err_mic_gain.cfg = cfg:i32("mic1_gain:  ", 8);
...
anc_config.drc_en.hbox_view = cfg:hBox {
    ...
    cfg:ispinView(anc_config.drc_en.cfg, 0, 7, 1),
    cfg:stLabel("(DRC使能，设置范围: 0 ~ 7，步进：1，默认值：0)"),
    cfg:stSpacer(),
};
...
-- 8-7 DCC选择
anc_config.dcc_sel.cfg = cfg:i32("dcc_sel:  ", 4);
anc_config.dcc_sel.cfg:setOSize(1);
anc_config.dcc_sel.hbox_view = cfg:hBox {
    cfg:stLabel(anc_config.dcc_sel.cfg.name .. TAB_TABLE[1]),
    cfg:ispinView(anc_config.dcc_sel.cfg, 0, 8, 1),
    cfg:stLabel("(DCC选择，设置范围: 0 ~ 8，步进：1，默认值：4)"),
    cfg:stSpacer(),
};
...
-- 8-49 reserved_1
anc_config.reserved_1.cfg = cfg:i32("reserved_1:  ", 0);
anc_config.reserved_1.cfg:setOSize(2);
anc_config.reserved_1.hbox_view = cfg:hBox {
    cfg:stLabel(anc_config.reserved_1.cfg.name .. TAB_TABLE[1]),
    cfg:ispinView(anc_config.reserved_1.cfg, 0, 10, 1),
    cfg:stLabel("保留配置"),
    cfg:stSpacer(),
};
...
-- 8-XXX 填充字节
-- anc_config.dummy_cfg.cfg = cfg:i32("dummy_cfg :  ", 100);
-- anc_config.dummy_cfg.cfg:setOSize(394);
-- anc_config.dummy_cfg.hbox_view = cfg:hBox {
    -- cfg:stSpacer(),
-- };
anc_config.dummy_cfg.cfg = cfg:fixbin("anc-dummy_cfg", 132, ""); -- default empty
