...
#define TEST_SEND_DATA_RATE         0  //测试上行的速率,AE02 notify
...
/* log_info("-----dueros dma config init\n"); */
/* connection_update_cnt = 0; */
/* set_ble_work_state(BLE_ST_NULL); */
...
/* extern u8 hci_le_get_acl_packet_num(u16 con_handle);
u8 hci_le_get_acl_packet_num(u16 con_handle)

u8 check_le_connect_idle(void)
{
	if(con_handle) {
		if(0 == hci_le_get_acl_packet_num(con_handle)) {
			return 1;
		} else {
			return 0;
		}
