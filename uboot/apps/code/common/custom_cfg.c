...
extern u16 Crc16(void *ptr, u32  len); //CRC校验
...
static u32 ex_cfg_read_data_from_flash(flash_cfg_read_param_t *param)
{
    ...
    if (*(u16 *)(temp_buf + param->cfg_crc_offset) != Crc16((u8 *)(temp_buf + param->cfg_data_offset), *(u16 *)(temp_buf + param->cfg_len_offset))) {
        err = EX_CFG_CRC_ERR;
        goto _EX_CFG_ERR_RET;
    }
...
u32 ex_cfg_get_content_api(u8 cfg_index, u8 *data, u16 len)
{
    ...
    case CFG_ITEM_SDK_TYPE:
        ...
        break;
    case CFG_ITEM_HID_PARAM:
        err = get_ex_cfg_info(MEMBER_OFFSET_OF_STRUCT(ex_cfg_t, hid_param_cfg), sizeof(hid_param_cfg_t), data, len);
        break;
    default:
