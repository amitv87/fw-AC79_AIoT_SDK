...
#define AP_SSID "WIFI_OTA_XXXXXX"      //配置 AP模式的SSID前缀
#define AP_PWD  ""                //配置 AP模式的密码

static struct wifi_store_info wifi_default_mode_parm = {0};

extern int bytecmp(unsigned char *p, unsigned char ch, unsigned int num);
extern void net_ota_wifi_scan(int clear);
...
{
    u8 addr3 = 1;
#ifdef CONFIG_HHF_PROJECT_ENABLE
    addr3 = 80;
#endif
    struct lan_setting lan_setting_info = {
        ...
        .WIRELESS_GATEWAY2  = addr3,
        .WIRELESS_GATEWAY3  = 1,

        .SERVER_IPADDR1  = 192,
        .SERVER_IPADDR2  = 168,
        .SERVER_IPADDR3  = addr3,
        .SERVER_IPADDR4  = 1,

        .CLIENT_IPADDR1  = 192,
        .CLIENT_IPADDR2  = 168,
        .CLIENT_IPADDR3  = addr3,
        .CLIENT_IPADDR4  = 2,
...
{
    do {
        u32 crc321 = JL_RAND->R64H;
        u32 crc322 = JL_RAND->R64L;
        memcpy(&macaddr[0], &crc321, sizeof(crc321));
        memcpy(&macaddr[4], &crc322, sizeof(crc321));
    } while (!bytecmp((u8 *)macaddr, 0, 6));
    macaddr[0] &= ~((1 << 0) | (1 << 1));
    printf("wifi use flash_uid+random mac\n");
    return 0;
}
struct wifi_store_info wifi_vm_info = {0};
int wifi_stored_mode_get(struct wifi_store_info *info)
{
    if (wifi_vm_info.mode) {
        memcpy(info, &wifi_vm_info, sizeof(struct wifi_store_info));
        return 1;
    }
    return 0;
}
...
static int wifi_event_callback(void *network_ctx, enum WIFI_EVENT event)
{
    ...
    case WIFI_EVENT_MODULE_INIT:
        ...
        ret = wifi_stored_mode_get(&wifi_default_mode_parm);

#ifdef CONFIG_WL_AP_ENABLE
        u8 mac_addr[6];
        char ssid[64];
        wifi_set_lan_setting_info();    //AP模式配置IP地址信息和DHCP池起始分配地址
        if (!ret) { //找不到VM存储AP模式和STA模式，则默认开启AP模式
            ...
            strncpy((char *)wifi_default_mode_parm.ssid[wifi_default_mode_parm.mode - STA_MODE],
                    (const char *)ssid, sizeof(wifi_default_mode_parm.ssid[wifi_default_mode_parm.mode - STA_MODE]) - 1);
            strncpy((char *)wifi_default_mode_parm.pwd[wifi_default_mode_parm.mode - STA_MODE],
                    (const char *)AP_PWD, sizeof(wifi_default_mode_parm.pwd[wifi_default_mode_parm.mode - STA_MODE]) - 1);
        }
#else
        if (!ret || wifi_default_mode_parm.mode != STA_MODE ||
            wifi_default_mode_parm.ssid[wifi_default_mode_parm.mode - STA_MODE] == NULL ||
            wifi_default_mode_parm.ssid[wifi_default_mode_parm.mode - STA_MODE] == 0) {
            printf("err in vm wifi info \n\n");
            cpu_reset();
        }
        printf("--->vm ssid = %s ,pdw = %s \n", wifi_default_mode_parm.ssid[wifi_default_mode_parm.mode - STA_MODE],
               wifi_default_mode_parm.pwd[wifi_default_mode_parm.mode - STA_MODE]);
        wifi_set_sta_connect_timeout(30);   //配置STA模式连接超时后事件回调通知时间
#endif
        wifi_set_default_mode(&wifi_default_mode_parm, 1, 0); //配置STA模式情况下,把默认配置SSID也存储起来,以后即使保存过其他SSID,也不会覆盖丢失,使用连接最优信号SSID策略的情况下可以匹配连接
        break;
...
int demo_wifi(void)
{
    return thread_fork("demo_wifi", 10, 1000, 0, 0, wifi_demo_task, NULL);
}
